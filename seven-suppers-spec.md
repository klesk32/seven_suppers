...
- Recipes no longer measure anything in packages. A recipe unit now has to be something you can weigh, count, or scoop, because scaling a package unit produced instructions nobody can follow: a 3-serving card used to ask for "0.75 cans (15 oz) canned black beans" and "0.5 loaves whole grain bread". The same cards now read "11.25 oz canned black beans" and "6 slices whole grain bread".
- 25 ingredients converted. Jars, cans, bottles, bags, and blocks became ounces at their shelf size (pesto 8, marinara 24, salsa 16, coleslaw 14, chips 10, tofu 14, beans 15, crushed tomatoes 28, diced 14.5, coconut milk 13.5, and the rest); bread became slices; green onions became a count of 6; and the taco and fajita seasoning packets became 6 tsp, matching the catalog's existing rule that dry spices are measured in teaspoons.
- Package identity moved from `SIZES` into `PACKS`, which now carries both the package and its size ("jar (8 oz)", "can (14.5 oz)"). `SIZES` is gone entirely, along with the `name` argument `formatQty` only needed to append it. `DISCRETE_UNITS` shrank to `head` and `bunch`, the two produce units that are still what you buy and what you use.
- Grocery lines keep the buy suggestion and drop the noise: "1 jar (8 oz) basil pesto (need 4 oz)" when there is leftover, and plain "1 can (15 oz) canned black beans" when the recipe uses exactly one package, instead of the old redundant "(need 1 can)".
- Two latent overstatements surfaced during the conversion and were fixed rather than carried across: barbecue sauce was `1 bottle` (about 18 oz of sauce for four servings, where the step only says "brush generously") and is now 6 oz, and whole grain bread was a whole loaf in two recipes and is now 8 slices.
- Left as they were: `head` (broccoli, romaine) and `bunch` (parsley). These are produce units you both buy and cook by, so "half a head of broccoli" is a real instruction, while "12 oz broccoli" would send you to a scale.
- Steps: the taco and fajita simmer water was keyed to "per seasoning packet used" and is now "for every 2 tablespoons of seasoning", which preserves the 2/3 cup ratio the 0.8.1 verification established while scaling with the new unit. The two tofu recipes now say "wrap the tofu" rather than "wrap the block".
- The validator gained the rule behind all of this: no recipe may measure an ingredient in a package unit, and every `PACKS` label that names a weight or volume package must state its size, so the shopper is never left guessing between two shelf sizes.
...
| `q` | number | Quantity |
| `u` | string | Unit in singular canonical form, e.g. `cup` not `cups` ("" for countable items). Display pluralizes via `UNIT_PLURALS` when quantity is not 1; unitless countables pluralize the name itself via `ITEM_PLURALS`. Units must be kitchen measures, never packages (0.22.0): `oz` for jarred, canned, bagged, and boxed goods, `slice` for bread, a bare count for green onions, with the package itself recorded only in `PACKS`. `head` and `bunch` stay, being produce units you both buy and cook by. Dry spices and sugar are always `tsp`, cornstarch always `tbsp` |
...
- Discrete-unit rounding (0.8.0): even without a `PACKS` entry, fractional quantities of whole purchasable things (`DISCRETE_UNITS`, since 0.22.0 just `head` and `bunch`) round up on the buy line with the true need shown: "1 head of broccoli (need half)". Staples never get buy lines; their section shows plain usage amounts.
- Expected package sizes (0.13.0, reworked in 0.22.0): the buy line names the package and the size the recipes assume ("1 can (28 oz) canned crushed tomatoes"), so a shopper facing a 14 oz and a 45 oz version knows which one the plan means. The size lives in the `PACKS` label; the `SIZES` map that used to hold it separately is gone, since recipe units are now kitchen measures and never packages.
